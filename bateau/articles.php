...
$config = [
    ...
    'n8n_webhook' => 'https://n8n.evolu8.fr/webhook/98cbf91e-e031-4391-8029-5e7e1ba12254'
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'publish') {
    ...
    if ($publishResult['httpCode'] === 200 || $publishResult['httpCode'] === 204) {
        ...
        header("Location: articles.php?id=$id&published=1");
        exit;
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['action'])) {
    ...
    // Redirect with success message
    header("Location: articles.php?id=$id&updated=1");
    exit;
}
...
<p>Utilisez ce formulaire pour envoyer une image liée à l’article. Choisissez si c’est une image à la une ou une image insérée dans le contenu.</p>
...
<form action="upload_images.php" method="POST" enctype="multipart/form-data">
    ...
    <select name="image_type" id="image_type">
        <option value="image_a_la_une">Image à la une</option>
        <option value="images_article">Image dans l’article</option>
    </select>
...
<?php if (isset($_GET['upload_success'])): ?>
    <div class="success-message">
        ✅ Image uploadée et ajoutée à l’article.
    </div>
<?php endif; ?>

<hr>
<h2>🖼️ Aperçu des images</h2>

<?php if (!empty($article['image_a_la_une'])): ?>
  <div>
    <strong>Image à la une :</strong><br>
    <img src="<?= htmlspecialchars($article['image_a_la_une']) ?>" style="max-width:300px;"><br>
    <!-- Affichage de l'URL -->
    <code style="word-break: break-all; display: block; margin: 5px 0;">
      <?= htmlspecialchars($article['image_a_la_une']) ?>
    </code>
    <form method="POST" action="manage_images.php" style="display:inline;">
      <input type="hidden" name="article_id"   value="<?= $id ?>">
      <input type="hidden" name="image_type"     value="image_a_la_une">
      <input type="hidden" name="action"         value="delete">
      <input type="hidden" name="image_url"      value="<?= htmlspecialchars($article['image_a_la_une']) ?>">
      <button type="submit">🗑 Supprimer</button>
    </form>
  </div>
<?php endif; ?>

<?php if (!empty($article['images_article'])):
    $imgs = json_decode($article['images_article'], true);
    if (is_array($imgs)): ?>
  <div style="margin-top:20px;">
    <strong>Images de l’article :</strong><br>
    <?php foreach ($imgs as $u): ?>
      <div style="display:inline-block; margin:10px; text-align:center;">
        <img src="<?= htmlspecialchars($u) ?>" style="max-width:150px;"><br>
        <!-- Affichage de l'URL -->
        <code style="word-break: break-all; display: block; font-size:12px; margin:5px 0;">
          <?= htmlspecialchars($u) ?>
        </code>
        <form method="POST" action="manage_images.php" style="margin-top:5px;">
          <input type="hidden" name="article_id"   value="<?= $id ?>">
          <input type="hidden" name="image_type"     value="images_article">
          <input type="hidden" name="action"         value="delete">
          <input type="hidden" name="image_url"      value="<?= htmlspecialchars($u) ?>">
          <button type="submit">🗑 Supprimer</button>
        </form>
      </div>
    <?php endforeach; ?>
